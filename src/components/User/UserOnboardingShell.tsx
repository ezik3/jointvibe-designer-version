...
import "@/components/Venue/venue-onboarding-shell.css";

interface UserOnboardingShellProps {
  children: ReactNode;
  step: number;
  backTo: string;
  wide?: boolean;
...
const UserOnboardingShell = ({ children, step, backTo, wide = false }: UserOnboardingShellProps) => {
  const currentStep = Math.max(1, Math.min(step, TOTAL_STEPS));

  return (
    <main className="venue-onboarding-shell user-onboarding-shell">
...
export default UserOnboardingShell;
