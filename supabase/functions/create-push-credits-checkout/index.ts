...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CREATE-PUSH-CREDITS-CHECKOUT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const { venueId, credits, reachTier, price, origin: clientOrigin, resumeDeal } = await req.json();
    ...
    logStep("Credits purchase details", { venueId, credits, reachTier, price });
    ...
    const successUrl = `${origin}/?checkout_return=wallet&credits_added=true&credits_expected=${credits}&reach_tier=${reachTier}${resumeDeal ? '&resume_deal=true' : ''}`;
    const cancelUrl = `${origin}/?checkout_return=wallet&credits_cancelled=true`;
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `Push Deal Credits — ${reachTier.charAt(0).toUpperCase() + reachTier.slice(1)} Reach`,
              description: `${credits} push notification credits (${reachTier} reach)`,
            },
